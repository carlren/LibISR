...
ISRColorTracker::ISRColorTracker(int nObjs,Vector2i imageSize, bool useGPU)
{
	nObjects = nObjs;
	ATb_Size = nObjs * 6;
...
ISRColorTracker::~ISRColorTracker()
{
	delete[] ATb_host;
	delete[] ATA_host;
}
...
void ISRColorTracker::computeSingleStepLM(float *step, float *ATA, float *ATb, float lambda, int dim)
{
...
void ISRColorTracker::computeSingleStepGN(float *step, float *ATA, float *ATb, int dim)
{
...
void ISRColorTracker::TrackObjects(ISRFrame *frame, ISRShapeUnion *shapeUnion, ISRTrackingState *trackerState, bool updateappearance)
{
	if (!initialized)
	{
	...
	for (int i = 0; i < 50; i++)
	{
		computeJacobianAndHessian(ATb_host, ATA_host, trackerState);
		//computeSingleStepGN(step, ATA_host, ATb_host, 6);
		computeSingleStepLM(step, ATA_host, ATb_host, 10, 6);
		
		trackerState->getPose(0)->applyIncrementalChangeToH(step);

		//for (int j = 0; j < 6; j++) step[j] = -step[j];
		//trackerState->getPose(0)->applyIncrementalChangeToInvH(step);

		
	}
}

void ISRColorTracker::computeMinmaxForRayCast(const Matrix4f& H, const Matrix3f& K, const Vector2i& imgsize)
{
	Vector3f ipts[8];
