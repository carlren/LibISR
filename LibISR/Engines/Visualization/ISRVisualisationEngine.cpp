...
void LibISR::Engine::ISRVisualisationEngine::updateMinmaxImage_fast(ISRFloat2Image* minmaximg, ISRFloat4Image* rgbd, const Matrix4f& H, const Matrix3f& K, const Vector2i& imgsize)
{
	Vector3f crns[8], ipts[8];
	Vector4i bb(imgsize.x, imgsize.y, 0, 0);
	float maxz = 0.0f, minz = 99999.9f;
	...
	float mu = 0.05f*(maxz - minz);

	Vector2f* minmax_ptr = minmaximg->GetData(false);
	Vector4f* rgbd_ptr = rgbd->GetData(false);

	for (int i = 0; i < imgsize.y; i++)  for (int j = 0; j < imgsize.x; j++)
	{
		if (j >= bb.x && j < bb.z && i >= bb.y && i < bb.w)
		{
			int id = i*imgsize.x + j;
			float z = rgbd_ptr[id].w;

			if (z>0 && z<maxz)
			{
				minmax_ptr[id].x = (z - mu) < minz ? minz : z - mu;
				minmax_ptr[i*imgsize.x + j].y = maxz;
			}
			else
			{
