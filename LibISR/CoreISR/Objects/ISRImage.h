...
#include "../Utils/LibISRCUDADefine.h"
#endif
	...
	namespace Objects
	{
		...
		*/
		...
		public:
			...
			*/
			explicit ISRImage(bool allocateGPU = false)
			{
			...
			*/
			ISRImage(Vector2i noDims, bool allocateGPU = false)
			{
				this->isAllocated = false;
				this->allocateGPU = allocateGPU;
				Allocate(noDims);
			...
			*/
			...
			void SetFrom(const ISRImage<T> *source, bool copyHost = true, bool copyDevice = false)
			{
			...
			ISRImage(const ISRImage&);
			ISRImage& operator=(const ISRImage&);
		};
	}
}
